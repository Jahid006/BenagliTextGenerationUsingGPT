...
class Tokenizer:
    def __init__(
        ...
        self.model_prefix = sentencepiece_path
    ...
    def train(
        ...
        cmd = (
            ...
            + f' --model_prefix={self.model_prefix}.{str(self.vocab_size)}'
            + ' --pad_id=0'
            + ' --unk_id=1'
            + ' --bos_id=2'
            + ' --eos_id=3'
    ...
    def tokenize(self, text, padding=False):
        ...
        padded_text, token_len = text, len(text)
        # attention_mask = [1]*token_len + [0]* (self.max_len-token_len)
        
        return {
                    "input_ids": padded_text,
                    "token_len": token_len,
    ...
    def get_id(self, token):
        return self.char_to_idx.get(self.token, self.unk_token_id())
